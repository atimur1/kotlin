...
class InflowSlicer(
    ...
    private fun KtParameter.processParameter() {
        ...
        val parameterDescriptor = analyze()[BindingContext.VALUE_PARAMETER, this]?.safeAs<ValueParameterDescriptor>() ?: return
        ...
        (function as? KtFunction)?.processCalls(parentUsage.scope.toSearchScope()) body@ {
            ...
            val argumentExpression = when {
                refElement is KtExpression -> {
                    ...
                    val resolvedArgument = resolvedCall.valueArgumentsByIndex?.get(parameterDescriptor.index) ?: return@body
